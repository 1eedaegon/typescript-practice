...
// 생명체들을 위한 항온시스템을 만드는 중 생명체에 대한 범주를 잡았다고 할 때, 
// 생명체는 생명과 체온이 있지만 살아남지 못하는 기준 온도는 종마다 각각 다르기 때문에 기준온도를 정하기 애매하다.
// 그렇다고 생명체 전체의 기준온도를 하나의 property로 잡을 순 없다. 
// 이 때 생명체 범주에서 기준 온도를 정하기 보다 생명체에 해당하는 각각의 범주에서 기준온도를 정하는 것이 낫다.
// `abstract class 생명체`로 만들고 abstract method로 `getLethalTemperature`로 구현하면
// `class Human extends 생명체 {}`를 할 수 있으면서 `class 생명체 {}`는 방지할 수 있다.

// 인터페이스는 선언적 확장과 다중상속을 통해 형태를 정의할 수 있다.
// 값 수준에서 정의할 수 있으므로 class, 객체, 인터페이스를 상속할 수 있다.
// 즉, 클래스를 정의할 때 implements로 여러개가 가능하므로 프로퍼티의 has-a 관계로 사용하면 좋다.